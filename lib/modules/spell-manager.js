...
function spellKey(name) {
  // Warning - the following line contains the Unicode character ’ ("proper apostrophe")
...
module.exports = class SpellManager extends ShapedModule {
  ...
  handleSpellCast(options) {
    ...
    if (usesAttr && usesAttr.get('current')) {
      currentVal = parseInt(usesAttr.get('current'), 10);
    }
    if (currentVal > 0) {
      return;
    }
  ...
  getSpellAttributesForImport(char, pronounInfo, newSpells, overwrite) {
    ...
    const jsonSpellsToAdd = _.chain(newSpells)
      .map((spell, index) => {
        ...
        // Warning - the following line contains the Unicode character ’ ("proper apostrophe")
        const existingSpell = spells[spell.name.toLowerCase()] || spells[spell.name.toLowerCase().replace(/’/g, '\'')];
        if (existingSpell) {
  ...
  getPronounInfo(character) {
    const gender = this.roll20.getAttrByName(character.id, 'gender', 'current', true);
